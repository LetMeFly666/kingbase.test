...
-- LET:原为PGSQL的any_value，不知是否对应金仓的any
SELECT any(v) FROM (VALUES (1), (2), (3)) AS v (v);
SELECT any(v) FROM (VALUES (NULL)) AS v (v);
SELECT any(v) FROM (VALUES (NULL), (1), (2)) AS v (v);
SELECT any(v) FROM (VALUES (array['hello', 'world'])) AS v (v);